...
const isCurrentQuestionAnswered = (current, answerKey) => {
  ...
  return (
    currentAnswer &&
    Array.isArray(currentAnswer.choices) &&
    currentAnswer.choices.length !== 0
  );
};
...
function Exam() {
  ...
  const checkResult = (questions) => {
    ...
    questions.forEach((questionObj) => {
      ...
      if (
        answerKey[questionObj.number - 1].questionNumber === questionObj.number
      ) {
  ...
  return (
    <div className="mt-5 certification-exam w-100">
      {userWarning && (
      ...
      <Card className="exam">
        ...
        <div className="exam-nav m-2 text-center">
          {exam.current != 1 && (
            <Button
              className="m-1"
              onClick={() => toggleQuestions('prev')}
              variant="primary"
            >
              Prev
            </Button>
          )}
          <Button className="m-1" onClick={submitAnswer}>Submit</Button>
          {!(exam.current >= exam.total) && (
            <Button
              ...
              variant="primary"
              disabled={
                !isCurrentQuestionAnswered(exam.current, exam.answerKey)
              }
            >
              Next
            </Button>
